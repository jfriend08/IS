...
def loadInterval2Frame(path, sr=22050, frameConversion=None):
  ...
  def f2f(x):
    def getFrame(myin):
      if myin < 0 or myin > frameConversion[-1][1][1] + 100:
        raise ValueError('frame index error')
      elif frameConversion[-1][1][1] <= myin <= frameConversion[-1][1][1] + 100:
        # just a buffer range
        print "buffer range"
        myin = frameConversion[-1][1][1]
